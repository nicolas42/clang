...
#define mv_length 8
typedef double* mv_t;
static unsigned int mv_size = 8 * sizeof(double);
...
static size_t mv_arena_size;
static size_t mv_arena_allocated;


// === Arena Allocator =============

void mv_arena_make(size_t n)
{
    mv_arena_size = 0;
    mv_arena_allocated = n * mv_size;
    mv_arena = (mv_t)malloc(mv_arena_allocated * mv_size);
}

mv_t mv_arena_allocate(void)
{
    mv_t offset = mv_arena + mv_arena_size * mv_size;

    // realloc on overflow
    if (mv_arena_size + mv_size > mv_arena_allocated){
        mv_arena_allocated *= 2;
        mv_arena = (mv_t)realloc(mv_arena, mv_arena_allocated * mv_size);
    }

    mv_arena_size += mv_size;
    return offset;
}
...
int mv_main(void)
{
    ...
    puts("r = mv_mul(2, mv_vector(1,1,0), mv_vector(1,1,0));");
    ...
    puts("r = mv_mul(2, mv_vector_spherical(1, TAU/8, TAU/4), mv_vector_spherical(1, TAU/8, TAU/4));");    
    ...
    puts("r = mv_mul(2, mv_vector(1,0,0), mv_vector(0,1,0));");    
    ...
    puts("r = mv_mul(2, mv_vector_spherical(1, TAU/8, TAU/8), mv_vector_spherical(1, TAU/4, TAU/4));");
    ...
    puts("\
    volume\n\
    Volume works great but you get a mv_trivector\n");

    puts("r = mv_mul(3, mv_vector(1,0,0), mv_vector(0,1,0), mv_vector(0,0,1));");    
    ...
    puts("r = mv_mul(3, mv_vector_spherical(1, TAU/8, TAU/4), mv_vector_spherical(1, 3*TAU/8, TAU/4), mv_vector(0,0,1));");
    r = mv_mul(3, mv_vector_spherical(1, TAU/8, TAU/4), mv_vector_spherical(1, 3*TAU/8, TAU/4), mv_vector(0,0,1));
    mv_print_trivector(r);
    puts(""); getchar();

    puts("A mv_trivector is also known as a pseudoscalar since it's a single number\n");
    ...
    puts("r = mv_mul(2, mv_vector(1,0,0), mv_bivector(1,0,0)); \n");
    ...
    puts("spinor = mv_mul(2, mv_vector(1,0,0), mv_vector_spherical(1, TAU/12, TAU/4));");
    puts("r = mv_mul(2, mv_vector(1,0,0), spinor);");
    ...
    puts("v = mv_vector(1,0,0);\n");
    puts("a = mv_vector_spherical(1, TAU/8, TAU/4);\n");
    puts("b = mv_vector_spherical(1, TAU/8, TAU/8);\n");
    puts("r = mv_mul(5, b,a,v,a,b);\n");
    ...
    puts("spinor1 = mv_mul(2, mv_vector(1,0,0), mv_vector_spherical(1,TAU/8,TAU/4))\n");
    puts("spinor2 = mv_mul(2, mv_vector(1,0,0), mv_vector_spherical(1,TAU/8,TAU/4))\n");
    puts("mv_mul(3, mv_vector(1,0,0), spinor1, spinor2);\n");
    ...
    puts("\n");
    puts("chirality???\n");
    puts("r = mv_mul(2, mv_trivector(3), mv_trivector(4));\n");
